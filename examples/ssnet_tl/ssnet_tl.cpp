...
void fetch_weight(ap_uint<512>* weight,
	...
	for (int i = 0; i < (total_oc); i++) {
		for (int k = 0; k < (total_ic); k++) {
			weight_stream.write(
					weight[k + i * (total_ic)].range(323, 0));
		}
	}
...
#pragma HLS INTERFACE m_axi port=weight offset=slave depth=4 bundle=gmem2
...
// actually is ap_int
	...
	fetch_weight(weight, weight_stream, ic_size, oc_size, in_fmap_h);
