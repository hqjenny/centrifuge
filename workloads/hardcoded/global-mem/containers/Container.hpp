...
namespace BCL {
  ...
  template <typename T>
  struct serialize <T, 0, std::enable_if_t<std::is_fundamental <T>::value>> :
  ...
  public:
    ...
    typedef typename std::result_of <Serialize(T)>::type serialized_type;
  ...
  class Container <T, Serialize,
    std::enable_if_t <is_serial_ptr<typename std::result_of <Serialize(T)>::type>::value>> {
  public:
    using TS = typename std::result_of <Serialize(T)>::type;
    ...
    void set(const T &val, uint64_t rank = BCL::rank()) {
      ...
      this->ptr = BCL::ralloc <SPT> (ptr.N, rank);
      if (this->ptr == nullptr) {
